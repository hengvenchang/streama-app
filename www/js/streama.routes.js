    ...
    .state('main', {
      ...
      resolve: {
        ...
        serverVersion: function(apiService, $rootScope) {
          ...
          return apiService.setup.getInfo(domain).then(
            function(response) {
...
function currentUserResolve(apiService, $state) {
  console.log(
    '%c currentUserResolve for ' + $state.current.name,
    'color: deeppink; font-weight: bold; text-shadow: 0 0 5px deeppink;'
  );
  return apiService.core.currentUser().then(
    function(response) {
      if (!response.data.id) {
        $state.go('setup');
      } else {
